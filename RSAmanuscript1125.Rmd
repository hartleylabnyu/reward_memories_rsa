...
---
```{r}
# RUN ALL CELLS IN THE FILE two_lines_function.Rmd BEFORE RUNNING THE BELOW CODE
```
...
setwd("/Users/susanbenear/Library/CloudStorage/GoogleDrive-susanbenear@gmail.com/My Drive/NYU/moterRSA")
...
summary(VTCersd1modelagelin)
anova(VTCersd1modelagelin, VTCersd1modelagesq)
...
# Trialwise VTC ERS
0. Greater VTC ERS is associated with better specific source memory on day 2
...
# Plot
vtcplot <- plot_model(VTCERStrialwiseplotting, type = "pred", terms = c("ERS [all]"), 
...
vtahpcplot <- ggplot(anthippRSAdatawbehavfullruns, aes(y = highvlowERS, x = VTA)) + 
...
```{r}
# Plot
agesqvtcers<-ggplot(VTCers.long, aes(y = similarity, x = ExactAge, colour = RewardType, group = RewardType)) +
  geom_point(size = 3) + # make each subject a different color dot
  stat_smooth(method = "glm",formula = y~poly(x, 2), fullrange = TRUE, aes(fill = RewardType), size = 3) + # add line of best fit
  #stat_smooth(method = "lm", se = TRUE) +
  xlab("Age") + # x axis label
...
# aHC ERS by age group instead of continuous age
...
agebinboxplot<- ggplot(anthippers.long, aes(
    x = RewardTypelab, y = similarity, fill = RewardTypelab)) +
