...
```{r loadFunc, include = FALSE}
source("two_lines_function.R")
```
...
2. Similar pattern of results when including only items that were retrieved on Day 1; significant effect of reward type (p = .011), marginal effect of age (p = .087)

```{r analyzeBehav2, echo = FALSE, warning = FALSE, message = FALSE}
# Day 2 specific associative memory - retrieved on Day 1 only
assochitd2ret.long = melt(behavdata,
                       id.vars = c("subid", "HighRewardStim", "AgeGroup", "ExactAge", "Gender"),
                       measure.vars = c("HighRewAssocRetrievedHitDay2", "LowRewAssocRetrievedHitDay2"),
                       variable.name = "assoc")
names(assochitd2ret.long)[names(assochitd2ret.long) == "value"] <- "mem"

# Code RewardType
findRew <- "(High|Low)"
assochitd2ret.long <- transform(assochitd2ret.long, RewardType = stringr::str_extract(assoc, findRew))

# Z-score age 
assochitd2ret.long$ageScaled <- scale(assochitd2ret.long$ExactAge) 

# Make reward variables factors
assochitd2ret.long$HighRewardStim <- as.factor(assochitd2ret.long$HighRewardStim)
assochitd2ret.long$RewardType <- as.factor(assochitd2ret.long$RewardType)

# Model
assocmemmodelretronly <- lmer(mem ~ RewardType * ageScaled + HighRewardStim +  (1|subid), control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=1e6)), contrasts = list(RewardType = "contr.sum", HighRewardStim = "contr.sum"), data = assochitd2ret.long) 
Anova(assocmemmodelretronly, type = "III")
```
...
print("Sensitivity analysis controlling for voxel number", quote = FALSE)
...
print("Sensitivity analysis removing 1 run subs", quote = FALSE)
...
* Greater VTC ERS is associated with better specific source memory on day 2

```{r trialwiseVTC, echo = FALSE, message = FALSE}
...
print("Plotting model", quote = FALSE)
Anova(VTCERStrialwiseplotting, type = "III")

# Plot
plot_model(VTCERStrialwiseplotting, type = "pred", terms = c("ERS [all]"), 
           title = "", line.size = 2, colors = "orange") +
  xlab("VTC Encoding-Retrieval Similarity") +
  ylab("Specific Source Memory Day 2 \n Predicted Probability") +
  xlim(-0.2, 0.6) +
  scale_y_continuous(limits = c(0.3, 0.8)) + # Set probability range
  rsa_theme
...
# Reward-sensitive VTA encoding activation / AntHipp ERS plot
ggplot(anthippRSAdatawbehavfullruns, aes(y = highvlowERS, x = VTA)) + 
  geom_point(size = 3.5, alpha = .9, color = "purple4") + 
  geom_smooth(method = lm, size = 2, color = "purple4", fill = "plum1", alpha = .3) + 
  labs(title = "", x = "VTA High > Low Reward Enc Activation", y = "High - Low Reward aHC ERS") + 
  rsa_theme
ggsave('figures/AntHipp_ERS_VTA_enc.png', width = 12, height = 8)
```

# Follow-Up and Supplemental Analyses

## ERS by non-linear age

Two-lines test

```{r ERS_nonlin, echo = FALSE, message = FALSE}
# Plot
ggplot(VTCers.long, aes(y = similarity, x = ExactAge, colour = RewardType, group = RewardType)) +
  geom_point(size = 3) + # make each subject a different color dot
  stat_smooth(method = "glm",formula = y~poly(x, 2), fullrange = TRUE, aes(fill = RewardType), size = 3) + # add line of best fit
...
VTCRSAdata$avgERS <- (VTCRSAdata$VTC.highrewardERS + VTCRSAdata$VTC.lowrewardERS) / 2
twolineplot <- twolines(avgERS~VTC.ExactAge, graph = 1, link = "gaussian", data = VTCRSAdata)
```

## AntHipp ERS by age group instead of continuous age

Age group model

```{r ERS_agegrp1, echo = FALSE, message = FALSE}
# Make age group variable a factor
anthippers.long$AgeGroup <- factor(anthippers.long$AgeGroup)

# Mixed-effects model
summary(anthippersinter <- lmer(similarity ~ RewardType * AgeGroup + HighRewardStim + (1|subid), REML = FALSE, control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=1e6)), data = anthippers.long))
```

Examine marginal means from reward x age group interaction

```{r ERS_agegrp2, echo = FALSE, message = FALSE}
# Get emmeans 
emm <- emmeans(anthippersinter, ~ RewardType * AgeGroup)

# Compare reward type within each age group
pairs(emm, by = "AgeGroup")
```

Plot ERS by age group

```{r ERS_agegrp3, echo = FALSE, message = FALSE}
# Relabel
anthippers.long <- anthippers.long %>%
  mutate(AgeGrouplab = factor(AgeGroup, levels = c(3, 2, 1), # order: kids, teens, adults
                           labels = c("Children", "Adolescents", "Adults")),
         RewardTypelab = factor(RewardType, levels = c("low", "high"),
                                                       labels = c("Low \nReward", "High \nReward")))

# Plot
ggplot(anthippers.long, aes(x = RewardTypelab, y = similarity, fill = RewardTypelab)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6) + 
  geom_jitter(
    aes(color = RewardTypelab),
    width = 0.2, alpha = 0.6, size = 3, stroke = 0.5, ) +
  ylab("Encoding-Retrieval Similarity") +
  xlab(NULL) +
  scale_fill_manual(values = c("snow4","gold"), guide = "none") + # guide = none removes the legend
  scale_color_manual(values = c("snow4","gold"), guide = "none") +
  facet_wrap(~AgeGrouplab, ncol = 3) + 
  rsa_theme + 
  theme(strip.background = element_rect(fill = "white", color = "black", linewidth = 0.8),
        strip.text = element_text(color = "black", face = "bold"),
        panel.border = element_rect(color = "black", fill = NA, linewidth = 0.8)) # outline each facet panel
```

## AntHipp ERS by correct/incorrect, separately for adults and children

```{r trialwiseaHC, echo = FALSE}
# Read in trial-wise VTC ERS data and change variable types
trialwiseaHCERS = read.csv("data/trialwiseERS_antHIPP_wmemoryday2.csv")
trialwiseaHCERS$subid <- as.factor(trialwiseaHCERS$subid)

# Merge ERS and demographic data
trialwiseaHCERSwdemog <- full_join(trialwiseaHCERS, demog, by = "subid")
trialwiseaHCERSwdemog$HighRewardStim = ifelse(as.numeric(as.character(trialwiseaHCERSwdemog$subid)) %% 2 == 0, "High Reward Cat - Face","High Reward Cat - Place")

# Z-score age and ERS and add age squared
trialwiseaHCERSwdemog$ageScaled <- scale(trialwiseaHCERSwdemog$ExactAge) # Scale age
trialwiseaHCERSwdemog$ageScaledsq <- (trialwiseaHCERSwdemog$ageScaled)^2
trialwiseaHCERSwdemog$ERSZ <- scale(trialwiseaHCERSwdemog$ERS) # Scale ERS

# Make reward variables factors
trialwiseaHCERSwdemog$HighRewardStim <- as.factor(trialwiseaHCERSwdemog$HighRewardStim)
trialwiseaHCERSwdemog$RewardType <- as.factor(trialwiseaHCERSwdemog$RewardType)

# Fit model for stats
aHCERStrialwiseacc <- lmer(ERSZ ~ ageScaled*memory*RewardType + HighRewardStim + (1|subid), data = trialwiseaHCERSwdemog)
summary(aHCERStrialwiseacc)
```

```{r ERS_ageaccuracy, echo = FALSE, message = FALSE}
# Define your groups
groups <- list("KidsTeens" = trialwiseaHCERSwdemog %>% filter(AgeGroup != 1),
               "Adults" = trialwiseaHCERSwdemog %>% filter(AgeGroup == 1))
...
for (g in names(groups)) {
  ...
  trialwiseaHC_avg <- trial_data %>%
    group_by(subid, RewardType, memory) %>%
    summarise(ERS = mean(ERS, na.rm = TRUE), .groups = "drop") %>%
    mutate(
      Group = interaction(RewardType, memory, sep = " & "),
      Group = factor(Group, levels = c("Low & Incorrect", "High & Incorrect",
                                       "Low & Correct", "High & Correct")),
      RewardTypelab = factor(RewardType, levels = c("Low", "High"),
                             labels = c("Low Reward", "High Reward")))

  reward_colors <- c("High Reward" = "gold", "Low Reward" = "gray")

  p <- ggplot(trialwiseaHC_avg,
              aes(x = RewardTypelab, y = ERS,
                  fill = RewardTypelab, color = RewardTypelab)) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "gray40") +
    geom_boxplot(outlier.shape = NA, alpha = 0.8, color = "black") +
    geom_jitter(width = 0.15, size = 1.8, alpha = 0.6) +
    scale_fill_manual(values = reward_colors) +
    scale_color_manual(values = reward_colors) +
    facet_wrap(~ memory, labeller = as_labeller(c("0" = "Incorrect", "1" = "Correct"))) +
    labs(x = "Reward Type", y = "ERS (Subject-Averaged)",
         title = paste("ERS by Reward Type and Accuracy —", g)) +
    rsa_theme + theme(legend.position = "none")
  
  print(p)  # Display each plot
  
  # Save plot
  ggsave(filename = paste0('figures/ERS_reward_accuracy_', g, '.png'), plot = p, width = 12, height = 8)
  
  }
```

## VTA high > low activation by age

```{r VTA_age, echo = FALSE, message = FALSE}
# Regression model
VTAage <- lm(VTA ~ ageScaled + HighRewardStim, data = anthippRSAdatawbehavfullruns)
summary(VTAage) 

# Plot
ggplot(anthippRSAdatawbehavfullruns, aes(y = VTA, x = ExactAge.x)) +
  geom_point(size = 3, color = VTA) + 
  geom_smooth(method = lm, size = 3, color = VTA, fill = VTA) + 
  xlab("Age") + # x axis label
  ylab("VTA High > Low Reward Enc Activation") + # y axis label
  rsa_theme
